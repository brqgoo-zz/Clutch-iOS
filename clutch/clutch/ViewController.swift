...
class ViewController: UIViewController, NFCNDEFReaderSessionDelegate{
    ...
    @IBOutlet weak var CollectiblesCollectionView: UICollectionView!
    var CollectiblesCollectionViewFlowLayout: UICollectionViewFlowLayout!
    
    private func setupCollectionView(){
        CollectiblesCollectionView.delegate = self
        CollectiblesCollectionView.dataSource = self
        let nib = UINib(nibName: "CollectiblesCollectionViewCell", bundle: nil)
        CollectiblesCollectionView.register(nib, forCellWithReuseIdentifier: "CollectiblesCollectionViewCell")
    }
    
    private func setupCollectionViewItemSize(){
        if CollectiblesCollectionViewFlowLayout == nil {
            
            let numberOfItemPerRow:CGFloat = 2
            let lineSpacing:CGFloat = 1
            let interItemSpacing:CGFloat = 1
            
            let width = (self.view!.bounds.width - (numberOfItemPerRow - 1) * interItemSpacing ) / numberOfItemPerRow
        
            let height = width
            
            CollectiblesCollectionViewFlowLayout = UICollectionViewFlowLayout()
            CollectiblesCollectionViewFlowLayout.itemSize = CGSize(width: width, height: height)
            CollectiblesCollectionViewFlowLayout.sectionInset = UIEdgeInsets.zero
            CollectiblesCollectionViewFlowLayout.scrollDirection = .vertical
            CollectiblesCollectionViewFlowLayout.minimumLineSpacing = lineSpacing
            CollectiblesCollectionViewFlowLayout.minimumInteritemSpacing = interItemSpacing
            
            CollectiblesCollectionView.setCollectionViewLayout(CollectiblesCollectionViewFlowLayout, animated: true)
...
extension ViewController: UICollectionViewDelegate, UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = CollectiblesCollectionView.dequeueReusableCell(withReuseIdentifier: "CollectiblesCollectionViewCell", for: indexPath) as! CollectiblesCollectionViewCell
        
        
        if indexPath.row == 0 {
            cell.CollectionCellImageView.image = UIImage(data: collectiblesArray[indexPath.row]["Icon"] as! Data)
            cell.CollectionCellLabel.text = (collectiblesArray[indexPath.row]["Symbol"] as? String)! + " #" + (collectiblesArray[indexPath.row]["Identifier"] as? String)!
        }
        else if indexPath.row == 1 {
            cell.CollectionCellImageView.image = UIImage(named: "curry")
            cell.CollectionCellLabel.text = "Curry #3635046"
        }
        else if indexPath.row == 2 {
            cell.CollectionCellImageView.image = UIImage(named: "hat")
            cell.CollectionCellLabel.text = "9FIFTY Snapback #2925876"
        }
        else if indexPath.row == 3 {
            cell.CollectionCellImageView.image = UIImage(named: "ball")
            cell.CollectionCellLabel.text = "Ball - Signed #3598002"
        }
        else if indexPath.row == 4 {
            cell.CollectionCellImageView.image = UIImage(named: "mask")
            cell.CollectionCellLabel.text = "Mask #3897132"
        }
        
        
        
        
        cell.CollectionCellView.layer.cornerRadius = 8
        cell.CollectionCellView.layer.shadowColor = UIColor.black.cgColor
        cell.CollectionCellView.layer.shadowOpacity = 0.03
        cell.CollectionCellView.layer.shadowOffset = .zero
        cell.CollectionCellView.layer.shadowRadius = 8
